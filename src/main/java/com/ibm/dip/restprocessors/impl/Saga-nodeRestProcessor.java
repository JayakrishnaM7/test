...
@Service
public class Saga-nodeRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Saga-nodeRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.saga-node.hosturl}")
	private String url;
	
	@Value("${spring.api.saga-node.context-path}")
	private String contextPath;

	@Value("${spring.api.saga-node.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Saga-nodeRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Saga-nodeRestProcessor.class.getName());
